class GithubService
  ...
  def self.repositories(username, oauth_token)
    response = conn.get "/users/#{username}/repos", { :access_token => oauth_token }
    JSON.parse(response.body, symbolize_names: true)
  end

  def self.events(username, oauth_token)
    response = conn.get "/users/#{username}/events", { :access_token => oauth_token }
    JSON.parse(response.body, symbolize_names: true)
  end

  def self.received_events(username, oauth_token)
    response = conn.get "/users/#{username}/received_events", { :access_token => oauth_token }
    JSON.parse(response.body, symbolize_names: true)
  end
